...
public class Add_productOwner {
	...
	while(col.next())
	{
		...
		//String[] userName= {"Aditi Shah","Alejandro Henao","Alexandria Forney","Alisha Couto","Amanda Rocco","Bala Vivekanandan","Christine Agudosi","Christine Bui","Corey Brown","Gustavo Becerra","Kaela Davis","Kevin Cleary","Khyati Shah","Lillian Chavez","Madhavi Patel","Madhavi Puppala","Madison Trauger","Mary Cheriyan","Meryam Attaalla","Nana Yaa Kodua","Nehali Shah","Nithya Pothireddy","Priscilla Eng","Ragha Anchala","Renee D'Alessio","Rengith Mathew","Tribhavan Singh"};
		String[] userName={"koduana1","henaoal2","forneal2","vivekba1","brownco7","becergu1","clearke3","anchara1","mathere5","puppama1","chaveli5","bradija1"};
		//String[] userName= {"Puppala Madhavi"};
		
		
		// Oncology ,"dalesre1"
		//String[] userName= {"koduana1","forneal2"};
		//String[] userName= {""};
		//String[] userName= getUsers(session);
		//System.out.println(pOwners);
		//String[] owners=pOwners.split(",");
		
		System.out.println("Adding user to product "+object_name);
		//document.appendString("config_product_owner", userName);
		//document.save();
		//document.setString("config_product_owner", userName);
		for(int u=0;u<userName.length;u++)
		{
			//String[] userfirstName = userName[u].split(" ");
			IDfUser user=(IDfUser) session.getObjectByQualification("dm_user where user_login_name like '%"+userName[u]+"%'");
				System.out.println("product "+object_name+" Adding user "+userName[u]+"");
				document.appendString("config_product_owner", user.getString("user_name"));
				//document.removeAll("config_product_owner");
				//document.remove("config_product_owner", u);
				//document.setString("config_product_owner", "");
		}
		//document.setString("config_product_owner", "dmadmin-p");
				//document.remove("config_product_owner", i);
				document.save();
			
	}
	}

	private static String[] getUsers(IDfSession session) throws DfException {
		String[] OncUsers;
		String query="select user_name from mi_config_product where business_unit='Oncology'";
		System.out.println("Query "+query);
		IDfCollection col = DFCHelper.executeQuery(session, query);
